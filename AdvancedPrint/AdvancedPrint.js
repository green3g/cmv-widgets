/* 
 * 
 * The main print dijit with additional functionality to handle the advanced print
 * http://resources.arcgis.com/en/help/main/10.1/index.html#/Tutorial_Advanced_high_quality_web_map_printing_exporting_using_arcpy_mapping/0057000000mq000000/
 */
define([
    'dojo/_base/declare',
    'dijit/_WidgetBase',
    ...
    'dojo/dom-construct',
    'dojo/dom-class',
    'dojo/text!./AdvancedPrint/templates/Print.html',
    'dojo/text!./AdvancedPrint/templates/PrintResult.html',
    ...
    'xstyle/css!./Print/css/Print.css'
        ...
        PrintTask, Memory, lang, array, topic, Style, domConstruct, domClass, printTemplate, printResultTemplate,
        LegendLayer, PrintTemplate, PrintParameters, esriRequest) {
...
// Main print dijit
    var PrintDijit = declare([_WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin], {
        ...
        pdfIcon: require.toUrl('gis/dijit/AdvancedPrint/images/pdf.png'),
        imageIcon: require.toUrl('gis/dijit/AdvancedPrint/images/image.png'),
        printTaskURL: null,
        printTask: null,
        postCreate: function () {
        ...
        print: function () {
            if (this.printSettingsFormDijit.isValid()) {
                ...
                var printLegendLayers = null;
                if (this.legendLayers !== null) {
                    var layerInfos = this.map.getLayersVisibleAtScale();
                    printLegendLayers = [];
                    function setParentIds(layer, visibleLayers, id) {
                        if (layer.layerInfos[id].hasOwnProperty('parentLayerId') &&
                                layer.layerInfos[id].parentLayerId !== -1 &&
                                visibleLayers.indexOf(layer.layerInfos[id].parentLayerId) === -1
                                ) {
                            visibleLayers.push(layer.layerInfos[id].parentLayerId);
                            setParentIds(layer, visibleLayers, layer.layerInfos[id].parentLayerId);
                    ...
                    array.forEach(layerInfos, lang.hitch(this, function (layer) {
                        if (this.legendLayers.indexOf(layer.id) !== -1) {
                            var legendLayer = new LegendLayer();
                            legendLayer.layerId = layer.id;
                            legendLayer.subLayerIds = layer.hasOwnProperty('visibleLayers') ?
                                    lang.clone(layer.visibleLayers) : null;
                            if (legendLayer.subLayerIds.hasOwnProperty('length')) {
                                array.forEach(legendLayer.subLayerIds, function (id) {
                                    setParentIds(layer, legendLayer.subLayerIds, id);
                                });
                            }
                            printLegendLayers.push(legendLayer);
                        }
                    }));
                }
                ...
                template.layoutOptions = {
                    ...
                    legendLayers: (layoutForm.legend.length > 0 && layoutForm.legend[0]) ? printLegendLayers : [],
